...
describe("fromGetAvailability", () => {
        ...
        disponibilidade: [
          {
            ...
            quantidades: [
              {
                medicamento:
                  "INSULINA HUMANA NPH 100 UI/ML SUSPENSÃO INJETÁVEL FAM 10 ML",
                quantidade: "18",
  ...
  it("should correctly match CANETA type insulin", () => {
    ...
    const insulinAvailability = result[0].availability.find(
      (a) => a.quantity > 0,
    );

    expect(insulinAvailability?.insulin.availableIn).toBe("CANETA");
  ...
  it("should correctly match AMPOLA type insulin", () => {
    ...
    expect(insulinAvailability?.insulin.availableIn).toBe("AMPOLA");
  ...
  it("should correctly match REFILL type insulin", () => {
    ...
    expect(insulinAvailability?.insulin.availableIn).toBe("REFILL");
  ...
  it("should correctly map availability levels", () => {
        ...
        disponibilidade: [
          {
            ...
            quantidades: [
              ...
              {
                medicamento:
                  "INSULINA HUMANA REGULAR 100 UI/ML SUSPENSÃO INJETÁVEL FAM 10 ML",
                quantidade: "20",
                nivelDisponibilidade: "médio",
              },
            ],
    ...
    const availableInsulins = result[0].availability.filter(
      (a) => a.quantity > 0,
    );

    const nphInsulin = availableInsulins.find(
      (a) => a.insulin.variant === "NPH",
    );
    const regularInsulin = availableInsulins.find(
      (a) => a.insulin.variant === "REGULAR",
    );
